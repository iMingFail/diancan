...
/**
 * @Author: Marte
 * @Date:   2018-05-07 15:37:37
 * @Last Modified by:   Marte
 * @Last Modified time: 2018-06-04 15:52:07
 */
...
if ($store['is_show'] != 1) {
    message('おはようございます。只今、お客様をお迎えする準備をしております。開店時刻までもうしばらくお待ち下さいませ。');
}
if ($mode == 1) { //店内
    unset($_SESSION['queue_orderid']);
...
AND deleted=0  AND find_in_set(".$week.",week) ORDER BY displayorder DESC, subcount DESC, id DESC limit 5");
...
status<>3 AND status<>-1 ORDER BY id DESC LIMIT 1", array(':from_user' => $from_user, ':weid' => $weid));
    ...
    // $jump_url = $this->createMobileUrl('classify', array('storeid' => $storeid, 'mode' => 3, 'selectdate' => $select_date, 'timeid' => $timeid, 'rtype' => 2), true);
...
$intelligent_goods =array();
foreach ($intelligents as $key => &$value) {
    $value['content'] = explode(',', $value['content']);
    foreach ($value['content'] as $k => $v) {
         $intelligent_goods[] = pdo_fetchall("SELECT title,id,thumb FROM " . tablename($this->table_goods) . " WHERE id = {$v} AND  storeid={$storeid} AND status = '1'
AND deleted=0");
        // var_dump($intelligent_goods);
    }
}
// var_dump($intelligent_goods);
// die;
...
$follow_url = $setting['follow_url'];
include $this->template($this->cur_tpl . '/mindex');