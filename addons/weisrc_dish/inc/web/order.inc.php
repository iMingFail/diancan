...
DESC LIMIT 1", array(':tid' => $id, ':uniacid' => $this->_weid));
 ...
 DESC LIMIT 1", array(':weid' => $weid, ':couponid' => $item['couponid']));
    ...
    $data['origOrderNum'] = $order['ordersn'];
    $data['busicd'] = 'REFD';
    //$data['respcd'] = '00';
    $data['inscd'] = $order['xl_code'];
    $data['mchntid'] = $order['xl_shopNum'];
    $data['terminalid'] = $order['xl_terminalNum'];
    $data['txndir'] = 'Q';
    ...
    if ($order['ispay'] == 1 || $order['ispay'] == 2 || $order['ispay'] == 4) { //已支付和待退款的可以退款
        if ($order['paytype'] == 2) { //微信支付
            ...
            message('操作成功！', $url, 'success');
             ...
             // file_put_contents('1112.txt',json_encode($data));
            $statusall['ispay']=3;
            $statusall['ordersn']=$orderNum;
            $statusall['origOrderNum']=$order['ordersn'];
            pdo_update($this->table_order, $statusall, array('id' =>$id));
            message('操作成功！', $url, 'success');
        }
    } else {
        message('操作失败！', '', 'error');
