...
if ($operation == 'detail') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_queue_order) . " WHERE dateline>UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE)) and  weid = :weid AND storeid =:storeid $condition ORDER BY id ASC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':weid' => $this->_weid, ':storeid' => $storeid));
    ...
    $queue_count = pdo_fetchall("SELECT queueid,COUNT(1) as count FROM " . tablename($this->table_queue_order) . " where dateline>UNIX_TIMESTAMP(CAST(SYSDATE()AS DATE)) and storeid=:storeid AND status=1 AND  weid = :weid GROUP BY queueid", array(':weid' => $this->_weid, ':storeid' => $storeid), 'queueid');
