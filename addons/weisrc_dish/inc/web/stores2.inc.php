...
//            pdo_update($this->table_stores, array('is_show' => 0), array('id' => $id, 'weid' => $weid));
        ...
        $ft_address= $_POST['ft_address'];
        $py_title= $_POST['py_title'];
        $py_address= $_POST['py_address'];
        if($en_title && $en_address){
            if (!empty($id)) {

                $arr = unserialize(file_get_contents('store_en.txt'));
                if(is_array($arr[$id])){
                    $arr[$id]['en_address']=$en_address;
                    $arr[$id]['en_title']=$en_title;
                }else{
                    $enarray1 =array("en_title"=>$entitle,'en_address'=>$en_address);
                    $arr[$id]= $enarray1 ;
                }
                // var_dump($arr);die;
                file_put_contents('store_en.txt',serialize($arr));
            }
        }
        if($jp_title && $jp_address){
            if (!empty($id)) {
                $jparr = unserialize(file_get_contents('store_jq.txt'));
                if(is_array($jparr[$id])){
                    $jparr[$id]['jp_address']=$jp_address;
                    $jparr[$id]['jp_title']=$jp_title;
                }else{
                    $jqenarray1 =array("jp_title"=>$jp_title,'jp_address'=>$jp_address);
                    $jparr[$id]= $jqenarray1 ;
                }
                file_put_contents('store_jp.txt',serialize($jparr));
            }
        }

        if($hw_title && $hw_address){
            if (!empty($id)) {

                $hwarr = unserialize(file_get_contents('store_hw.txt'));
                if(is_array($hwarr[$id])){
                    $hwarr[$id]['hw_address']=$hw_address;
                    $hwarr[$id]['hw_title']=$hw_title;
                }else{
                    $hwenarray1 =array("hw_title"=>$hw_title,'hw_address'=>$hw_address);
                    $hwarr[$id]= $hwenarray1 ;
                }
                file_put_contents('store_hw.txt',serialize($hwarr));
            }
        }

        if($ft_title && $ft_address){
            if (!empty($id)) {

                $ftarr = unserialize(file_get_contents('store_ft.txt'));
                if(is_array($ftarr[$id])){
                    $ftarr[$id]['ft_address']=$ft_address;
                    $ftarr[$id]['ft_title']=$ft_title;
                }else{
                    $ftenarray1 =array("ft_title"=>$ft_title,'ft_address'=>$ft_address);
                    $ftarr[$id]= $ftenarray1 ;
                }
                file_put_contents('store_ft.txt',serialize($ftarr));
            }
        }

         if($py_title && $py_address){
            if (!empty($id)) {

                $pyarr = unserialize(file_get_contents('store_py.txt'));
                if(is_array($pyarr[$id])){
                    $pyarr[$id]['py_address']=$py_address;
                    $pyarr[$id]['py_title']=$py_title;
                }else{
                    $pyenarray1 =array("py_title"=>$py_title,'py_address'=>$py_address);
                    $pyarr[$id]= $pyenarray1 ;
                }
                file_put_contents('store_py.txt',serialize($pyarr));
            }
        }
...
//    $users = pdo_fetchall("SELECT * FROM " . tablename($this->table_account) . " WHERE weid=:weid ORDER BY id DESC", array(':weid' => $weid), 'uid');
    ...
    if ($_W['role'] == 'manager' || $_W['isfounder']) {
        if (!empty($_GPC['dispatchprice']) || !empty($_GPC['sendingprice'])) {
            $data = array();
            if (!empty($_GPC['dispatchprice'])) {
                $data['dispatchprice'] = floatval($_GPC['dispatchprice']);
            }
            if (!empty($_GPC['sendingprice'])) {
