...
//                $content .= '备注:' . $order['remark'] . $huanhang;
            ...
            $content2 .= "総数量:" . $order['totalnum'] . "   总价:" . number_format($order['totalprice'], 2) . "元\n";
            ...
            if ($goods) {
                ...
                $content1 = $huanhang . "商品リスト{$viptip}{$appendtip}" . $huanhang;
                $content1 .= "-------------------------" . $huanhang;
            }
    ...
    public function _yilianyunSendFreeMessage($orderid = 0, $position_type = 0)
    {
        ...
        foreach ($settings as $item => $value) {
            ...
            $content2 .= "<FS>総数量:" . $order['totalnum'] . "   总价:" . number_format($order['totalprice'], 2) . "元</FS>\n";
            if ($dining_mode != 4 && !empty($order['meal_time'])) {
    ...
    public function feieSendFreeMessage($orderid = 0, $position_type = 0)
    {
        ...
        $content = "番号:" . $order['ordersn'] . "<BR>";
        if ($dining_mode == 4) {
            $content .= '注文番号:' . $order['quicknum'] . "<BR>";
        }
        $content .= '注文種類:' . $ordertype[$dining_mode] . "<BR>";
        $content .= '所属门店:' . $store['title'] . "<BR>";
        // $content .= '所属店舗:' . $paytype[$order['paytype']] . "(" . $paystatus . ")<BR>";
        $content .= '注文時間:' . date('Y-m-d H:i:s', $order['dateline']) . "<BR>";
        if ($dining_mode == 3) {
            $tablezones = pdo_fetch("SELECT * FROM " . tablename($this->table_tablezones) . " where weid = :weid AND id=:id LIMIT 1", array(':weid' => $weid, ':id' => $order['tablezonesid']));
            $content .= "席種類：{$tablezones['title']}<BR>";
        }
        if (!empty($order['tables'])) {
            $content .= '席詳細:<B>' . $this->getTableName($order['tables']) . "</B><BR>";
        }
        if (!empty($order['counts'])) {
            if($order['counts']>10){
                $order['counts']="10人以上";
            }
            $content .= '人数:' . $order['counts'] . "<BR>";
        }
        if (!empty($order['remark'])) {
            $content .= 'その他:' . $order['remark'] . "<BR>";
        }
        $content .= '住所:' . $store['address'] . "<BR>";
        $content .= '電話番号:' . $store['tel'] . "<BR>";

        $content2 = "-------------------------<BR>";

      $content2 .= "総数量:" . $order['totalnum'] . "   合計:" . number_format($order['totalprice'], 2) . "元<BR><BR>";
        if ($dining_mode != 4 && !empty($order['meal_time'])) {
            $content2 .= '到着時間:' . $order['meal_time'] . "<BR>";
        }
        if (!empty($order['username'])) {
            $content2 .= '名前:' . $order['username'] . "<BR>";
        }
        if (!empty($order['tel'])) {
            $content2 .= '携帯電話:' . $order['tel'] . "<BR>";
        }
        if (!empty($order['address'])) {
            $content2 .= '住所:' . $order['address'] . "<BR>";
        }
        $content2 .= "-------------------------<BR>";
        ...
        foreach ($settings as $item => $value) {
            ...
            if ($order['status'] == -1) {
                ...
                $cancelcontent = '^キャンセル' . "\n";
                $cancelcontent .= '番号:' . $order['ordersn'];
                $feie_content = array(
                    'sn' => $value['print_usr'],
            ...
            if ($goods) {
                ...
                $content1 = "商品リスト{$viptip}{$appendtip}<BR>";
                $content1 .= "-------------------------<BR>";
            }
            ...
            if ($value['is_print_all'] == 1) {
                ...
                $content .= '注文時間:' . date('Y-m-d H:i:s', $order['dateline']) . "<BR>";
                if (!empty($order['tables'])) {
                    $content .= '<B>席詳細:</B><DB>' . $this->getTableName($order['tables']) . "</DB><BR>";
                }
                if (!empty($order['remark'])) {
                    $content .= '<DB>その他:' . $order['remark'] . "</DB><BR>";
                }
                foreach ($order['goods'] as $v) {
                    ...
                    $content1 = '';
                    $content1 .= "------------------------------------------------<BR>";
                    $content1 .= '<B>注文内容:' . $v['title'] . "</B><BR>";
                    $content1 .= '<B>商品数:' . $goodsid[$v['id']]['total'] . $v['unitname'] . "</B><BR>";
                    $money = $goodsid[$v['id']]['price'];
                    $content1 .= '<B>単価:' . number_format($money, 2) . "元</B><BR>";
                    $content1 .= "------------------------------------------------<BR>";
    ...
    public function feiyinSendFreeMessage($orderid = 0, $position_type = 0)
    {
        ...
        foreach ($settings as $item => $value) {
            ...
            if ($goods) {
                ...
                $content1 = "商品リスト{$viptip}{$appendtip}\n";
                $content1 .= "-------------------------\n";
            }
...
//                message('没有有效的支付方式, 请联系网站管理员.');
    ...
    public function doMobilexlpay(){
        $order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " WHERE ordersn = :ordersn", array(':ordersn' => $_GET['orderNum']));
        $orderid = $order['id'];
        if($order['ispay']==1){
            echo "已支付";die;
        }
        $now = time();
        if($now-$order['dateline']>=60){
           message('订单超时!');die;
        }
        //查找昵称 判断用户是否正常
        $userInfo = pdo_fetch("SELECT * FROM " . tablename($this->table_fans) . " WHERE from_user = :from_user", array(':from_user' =>$order['from_user']));
        if($userInfo['status'] !=1){
            echo "用户不可法";die;
        }
        // $nickname = $userInfo['nickname'];
        //根据订单店铺id查找讯联key  xl_terminalNum xl_shopNum
        $store = pdo_fetch("SELECT id,xl_shopNum,xl_terminalNum,xl_key,xl_code FROM " . tablename($this->table_stores) . " WHERE id = :storeid", array(':storeid' =>  $order['storeid']));
        // var_dump($store);die;

        header("Content-type: text/html; charset=utf-8");
        $miyao=$store['xl_key'];
        $mchntid = $store['xl_shopNum'];
        $xl_terminalNum = $store['xl_terminalNum'];
        $data['version'] = "2.2";
        ...
        $data['frontUrl'] = 'http://'.$_SERVER['SERVER_NAME'].'/payment/xlpay/come.php';
        $data['mchntid'] = $mchntid;
        $data['terminalid'] = $xl_terminalNum;
        // $data['inscd'] = $store['xl_code'];
        $data['chcd'] = 'WXP';
        $data['paylimit'] = 'credit';
        // $data['timeStart'] = date('YmdHis');
        // $data['timeExpire'] ="".(date('YmdHis')+600);
        $data['txamt'] = str_pad($order['totalprice']*100, 12, "0", STR_PAD_LEFT);
        ksort($data);
        $str = '';
        foreach($data as $k=>$v){
            if($str!=''){
        ...
        // $url ="https://showmoney.cn/scanpay/unified?data=".base64_encode(json_encode($data));
        // file_put_contents("url.txt",$url);
        //
        $statusall =array();
        $statusall['status']=-2;
        $statusall['xl_shopNum']=$mchntid;
        $statusall['xl_terminalNum']=$xl_terminalNum;
        $statusall['xl_key']=$miyao;
        $statusall['xl_code']=$store['xl_code'];

        file_put_contents('12.txt',$data);
        pdo_update($this->table_order, $statusall, array('ordersn' => $data['orderNum']));
        $this->addOrderLog($order['id'], $userInfo['nickname'], 1, 1,12);
        // header("Location:https://showmoney.cn/scanpay/unified?data=".base64_encode(json_encode($data)));

        header("Location:http://sandbox.showmoney.cn/scanpay/unified?data=".base64_encode(json_encode($data)));

    }

    public function doMobileoutline(){
        $orderid =$_POST['orderid'];
        $order = pdo_fetch("SELECT * FROM " . tablename($this->table_order) . " WHERE id = :id", array(':id' =>  $orderid));
        if($order){
            $orderid = $order['id'];
            $orders =array();
            $orders['tables'] =$order['tables'];
            $orders['storeid'] =$order['storeid'];
            $orders['weid'] =$order['weid'];
            $id = pdo_update($this->table_order, array('ispay'=>1), array('id' => $orderid));
            if($id){
                $log = $this->addOrderLog($order['id'], $userInfo['nickname'], 1, 1,2);
...
print_usr=:print_usr) AND storeid = :storeid {$condition} ORDER BY id DESC limit 1", array(':storeid' => $storeid, ':print_usr' => $usr));
        ...
        $content2 = "----------------------------\n";
        $content2 .= "%10総数量:" . $order['totalnum'] . "   总价:" . number_format($order['totalprice'], 2) . "元\n%00";
        if (!empty($order['username'])) {
            $content2 .= '姓名:' . $order['username'] . "\n";
        }
        if (!empty($order['tel'])) {
