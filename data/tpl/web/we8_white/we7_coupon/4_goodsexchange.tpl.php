...
<ul class="nav nav-tabs">
	<li <?php  if($op == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWeburl('goodsexchange', array('op' => 'display'));?>">管理实物兑换</a></li>
	<li <?php  if($op == 'post' && !$id) { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWeburl('goodsexchange', array('op' => 'post'));?>">添加实物兑换</a></li>
	<?php  if($op == 'post' && $id) { ?><li class="active"><a href="<?php  echo $this->createWeburl('goodsexchange', array('id' => $id, 'op' => 'post'));?>">编辑实物兑换</a></li><?php  } ?>
	<li <?php  if($op == 'deliver') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWeburl('goodsexchange', array('op' => 'deliver'));?>">发货记录</a></li>
	<?php  if($op == 'receiver' && $id) { ?><li class="active"><a href="<?php  echo $this->createWeburl('goodsexchange', array('id' => $id, 'op' => 'receiver'));?>">编辑收货人信息</a></li><?php  } ?>
</ul>
<?php  if($op == 'post') { ?>
<style>
		...
		<div class="panel-body">
			<form action="./index.php" method="get" class="form-horizontal" role="form">
				...
				<div class="form-group">
					...
					<div class="col-sm-6 col-lg-8 col-xs-12">
						<select class="form-control" name="exid">
							<?php  if(is_array($exchanges_info)) { foreach($exchanges_info as $exchange) { ?>
							<option value="<?php  echo $exchange['id'];?>" <?php  if($_GPC['exid'] == $exchange['id']) { ?>selected<?php  } ?>><?php  echo $exchange['title'];?></option>
							<?php  } } ?>
						</select>
					</div>
